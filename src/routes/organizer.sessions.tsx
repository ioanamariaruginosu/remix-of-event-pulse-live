  ...
  return (
    <div className="p-8 lg:p-12 space-y-8 max-w-6xl">
      <div className="flex items-end justify-between">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-widest text-foreground/40 mb-2">Day 1 · May 22</div>
        ...
        {sessions.map((s) => {
          ...
          return (
            <div key={s.id} className="p-5 ring-1 ring-border rounded-2xl grid md:grid-cols-[120px_1fr_auto] gap-4 items-start hover:ring-primary/30 transition-colors">
              <div>
                <div className="font-mono text-sm font-bold">{s.time}</div>
                <div className="text-[10px] font-mono uppercase tracking-widest text-primary mt-1">{room?.name}</div>
              </div>
              <div>
                ...
                <div className="flex gap-1.5 mt-3 flex-wrap">
                  {s.topics.map((t) => (
                    <span key={t} className="px-2 py-0.5 bg-foreground/5 text-[10px] font-mono font-bold uppercase tracking-widest rounded">
                      {t}
                    </span>
                  ))}
                </div>
