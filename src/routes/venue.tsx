  ...
  return (
    <div className="min-h-screen bg-foreground text-white p-8 overflow-hidden">
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="font-display italic text-[10px] uppercase tracking-widest text-white/40 mb-1">Venue Spectacle · Live</div>
          <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight">
            {event.name}
          </h1>
        </div>
        <div className="text-right">
          <div className="font-display italic text-[10px] uppercase tracking-widest text-white/40 mb-1">Online</div>
      ...
      <div className="grid lg:grid-cols-[1fr_280px] gap-6">
        <div className="bg-background/5 rounded-3xl ring-1 ring-white/10 aspect-[16/10] overflow-hidden relative">
          ...
          <div className="absolute bottom-6 left-6 right-6 flex items-center justify-between">
            <div className="px-4 py-2 bg-foreground/80 backdrop-blur rounded-full text-[10px] font-display italic font-bold uppercase tracking-widest flex items-center gap-2 text-white">
        ...
        <div className="space-y-3">
          <div className="font-display italic text-[10px] uppercase tracking-widest text-white/40">Live stream</div>
          {tickerEvents.slice(0, 6).map((e, i) => (
            <div
              key={`${tick}-${i}`}
              className="p-3 bg-white/5 ring-1 ring-white/10 rounded-xl text-[11px] font-display italic font-bold tracking-tight animate-[fade-in-up_0.6s_var(--ease-out-expo)]"
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3 mt-6">
        {rooms.map((r) => (
          <div key={r.id} className="p-4 bg-white/5 ring-1 ring-white/10 rounded-2xl">
            <div className="text-[10px] font-display italic tracking-tight normal-case text-white/40 truncate">{r.name}</div>
